...
/**
 * Detecta o nome do MMC.
 *
 * Estratégia primária (determinística): se o markdown tem POVs com contagens
 * diferentes de `#` (ex.: `### ✦ FMC` e `#### ✦ MMC`), o grupo com MAIS hashes
 * é o MMC. Esta convenção vem do prompt do tradutor — `####` marca POV masculino.
 *
 * Fallback: quando todos os POVs estão no mesmo nível de hashes (script
 * importado de PDF que perdeu a estrutura), usa heurística de frequência —
 * o menos frequente é o MMC, desde que apareça ≥ 2 vezes e não haja empate.
 */
...
/**
 * Encontra ranges de linhas dentro de blocos `<<intimate>> ... <</intimate>>`.
 * As linhas dos próprios marcadores ficam FORA do range (não devem ser pintadas
 * nem renderizadas — quem chama precisa pular essas linhas via isIntimateMarker).
 */
